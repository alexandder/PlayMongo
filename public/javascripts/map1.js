...
function draw2() {
	...
	d3.json('/assets/javascripts/data.json', function(collection) {
		json = collection;
		
		states.selectAll('path').data(collection.features).enter().append(
				'path').attr('d', d3.geo.path().projection(projection)).attr(
				'id', function(d) {
					return d.properties.name.replace(/\s+/g, '')
				}).classed('none_active', true).on('click', click);
		function click(d) {
			var x = -150, y = 0, k = 1.0;
			if (d && centered !== d) {
			...
			states.selectAll("path").classed("active_loc",
					centered && function(d) {
						return d === centered;
					});
